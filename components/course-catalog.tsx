  ...
  const visible = useMemo(
    () =>
      courses.filter((course) => {
        const textMatch = `${course.code} ${course.title} ${course.description}`
          .toLowerCase()
        ...
        const subjectMatch =
          ...
          (active === "Mathematics" &&
            ["MTH", "STA"].some((prefix) => course.code.startsWith(prefix))) ||
    ...
    <section className="catalog-section">
        ...
        {visible.map((course, index) => (
          ...
          >
            ...
            <div className="catalog-card-footer">
              <span>
                <Check size={14} aria-hidden="true" /> {course.lessons}{" "}
                {course.lessons === 1 ? "reading" : "readings"}· {course.status}
              </span>
              <Link
                href={
                  course.code === "CS102"
                    ? "/lecture"
                    : `/lecture?course=${course.code}`
                }
      ...
      {visible.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Try another title, code, or subject.</p>
        </div>
      ) : null}
    </section>
  );
